...
def ensure_job_timeouts(text: str, protected: bool) -> tuple[str, bool]:
    ...
    for index, start in enumerate(starts):
        ...
        if re.search(r"(?m)^    uses:\s*", block_text) or "runs-on:" not in block_text:
            continue
...
def main() -> int:
    ...
    print(f"repaired {path}: {', '.join(notes)}" if changed else f"no safe policy repair needed for {path}")
    return 0
