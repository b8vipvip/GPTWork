...
fn capability_lease_required(id: Value) -> Value {
    json!({
        ...
        "error": {
            "code": "capability_lease_required",
            "message": "A server-issued capability lease is required for protected operations",
            "messageZhCn": "受保护操作需要服务端签发的能力租约",
            "messageEn": "A server-issued capability lease is required for protected operations"
        }
    })
}
...
fn private_engine_error(id: Value, detail: String) -> Value {
    json!({ "id": id, "ok": false, "protocolVersion": PRIVATE_ENGINE_PROTOCOL_VERSION, "error": {
        "code": "private_engine_unavailable", "message": detail,
        "messageZhCn": "私有核心组件当前不可用", "messageEn": "Private core component is unavailable"
    }})
}
...
mod tests {
    ...
    fn state() -> Arc<AppState> {
        ...
        AppState::initialize(ConfigStore::at(directory.keep())).unwrap()
    }
    ...
    #[test]
    fn capabilities_advertise_fail_closed_lease_boundary() {
